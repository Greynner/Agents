"""
Hannah MS Agent - Agente especializado en pruebas de microservicios (API REST)
...
axiom_logger = AxiomLogger(source="hannah_ms_agent")
  ...
  "matrix": [
    {
      ...
      "Datos de entrada": "payload o parámetros",
      "Código esperado": "200",
...
Incluye al menos 6 escenarios: exitoso (2xx), error 400, error 404, error 500, payload inválido y respuesta vacía.
...
def generate_ms_test_cases(requirement: str):
    ...
    try:
        ...
        axiom_logger.log_info(
            "Casos MS generados exitosamente",
...
async def analizar_requerimiento_ms(request: Request):
    ...
    try:
        ...
        if not requirement:
            axiom_logger.log_warning("Solicitud MS sin requerimiento")
        ...
        if len(requirement) > MAX_REQUIREMENT_LENGTH:
            axiom_logger.log_warning(
                "Requerimiento MS demasiado largo",
        ...
        axiom_logger.log_info(
            "Solicitud MS recibida desde frontend",
            {"requirement_length": len(requirement)},
        )
